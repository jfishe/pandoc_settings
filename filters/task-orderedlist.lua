...
function M.ballot_box (format)
  if is_html(format) then
    return pandoc.RawInline(
      'html',
      '<input type="checkbox" class="task-orderedlist-item-checkbox" disabled />'
    )
...
function M.ballot_box_with_check (format)
  if is_html(format) then
    return pandoc.RawInline(
      'html',
      '<input type="checkbox" class="task-orderedlist-item-checkbox" checked disabled />'
    )
  elseif format == 'gfm' then
    -- GFM includes raw HTML
...
M.css_styles = [[
<style>
  .task-orderedlist-item {
    list-style-type: none;
  }
  .task-orderedlist-item-checkbox {
    margin-left: -1.6em;
  }
</style>
]]
...
--- Convert Github- and org-mode-style task markers in an OrderedList.
function M.modifyOrderedList (list)
  if not is_html(FORMAT) then
    ...
    local res = List:new{pandoc.RawBlock('html', '<ol>')}
    for _, item in ipairs(list.content) do
      ...
      if blk == item[1] then -- does not have a todo marker
        ...
        res[#res + 1] = pandoc.RawBlock('html', '<li class="task-orderedlist-item">')
        item[1] = blk
      end
      res:extend(item)
      res[#res + 1] = pandoc.RawBlock('html', '</li>')
...
M[1] = {
  OrderedList = M.modifyOrderedList,
