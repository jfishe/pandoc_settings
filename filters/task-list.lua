-- Convert task markers in BulletList items to format-appropriate checkbox
-- renderings.
--
-- Usage:
--   pandoc --lua-filter=task-list.lua --to=FORMAT input.md
--
-- This filter rewrites bullet list items that start with GitHub- or org-style
...
--
-- The filter only rewrites the first block in each bullet-list item, so task
